...
function () {
  ...
  _createClass(ProcessHtml, [{
    ...
    value: function links(_links) {
      var source = ''; // A function to test an href against options.ignoreLinks and options.ignoreLinksFromPartialMatches
      ...
      if (this.options.ignoreLinksFromPartialMatches) {
        var partials = this.options.ignoreLinksFromPartialMatches;
        ignoreConditions = ignoreConditions.concat(function (resource) {
          return partials.some(function (partial) {
            return resource.indexOf(partial) > -1;
          });
        });
      }

      if (ignoreConditions.length > 0) {
        shouldIgnore = (0, _RuleSet.normalizeCondition)(ignoreConditions);
      } else {
      ...
      if (this.options.ignorePathReWrite) {
        shouldRewrite = (0, _RuleSet.normalizeCondition)({
          not: this.options.ignorePathReWrite
        });
      } else {
        shouldRewrite = function shouldRewrite() {
          return true;
        };
      }
